...
- Select each built-in song and confirm editing is locked by default.
- On an editing-locked built-in song, long-press locked edit controls such as song name, first measure number, insert, delete, grouping, and measure numerator/denominator. Confirm each unlocks temporarily, the held control immediately performs its edit action where applicable, each edit restarts the timeout, and editing locks again after 3 seconds with no edits.
- Create a new song.
- Duplicate an editing-locked song and confirm the copy is editable.
- Expand the inline song picker and switch between songs.
- Confirm editing-locked songs show a lock indicator in the inline song picker.
...
- Reset a built-in song and confirm it returns to its original sequence and editing-locked state.
...
- On an editing-locked song, long-press a numerator or denominator, keep holding, then drag vertically. Confirm editing temporarily unlocks and the same held drag edits the value without needing a second touch.
