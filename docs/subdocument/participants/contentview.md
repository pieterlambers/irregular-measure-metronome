...
- Lets the user long-press locked composition-edit controls, including song name, first measure number, insert/delete, grouping, and measure numerator/denominator controls, to temporarily unlock a read-only song for editing. Where the held control has an immediate edit action, the same long press continues into that action after unlock, such as focusing text fields, opening grouping, inserting/deleting, or beginning signature drag editing. It automatically locks again after 3 seconds without composition edits. The explicit lock button remains manual and does not auto-lock after unlocking.
